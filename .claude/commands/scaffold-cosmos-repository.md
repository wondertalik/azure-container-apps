...
3. Read an existing repository in `src/Users.Infrastructure.CosmosDb/Repositories/` as a pattern reference.
...
5. Register the repository in `src/Users.Infrastructure.CosmosDb/DependencyInjection.cs`:
   - `services.AddSingleton<I{Entity}Repository, {Entity}Repository>();`
6. Add the container configuration to `src/Users.Infrastructure.CosmosDb/CosmosDbExtensions.cs` inside `ConfigureUsersCosmosDbContainers`:
   - `.Configure<{Entity}>(o => o.WithName(UsersCosmosDbConstants.{Plural}.Name).WithPartitionKeyPath(UsersCosmosDbConstants.{Plural}.PartitionKey).WithPrimaryKey(e => e.{PrimaryKeyProp}).WithPartitionKey(e => e.{PartitionKeyProp}))`
   - If the container is new, also add it to `src/Users.Authorization.Constants/UsersCosmosDbConstants.cs`.
7. Run `dotnet build src/Users.Infrastructure.CosmosDb/Users.Infrastructure.CosmosDb.csproj` to verify.
